...
## Премоляры и моляры: анатомия по правилам

Отражённый симметричный зуб бывает неудачным: наклонён, стёрт, плохо
отсканирован. Поэтому жевательные зубы по умолчанию строятся моделью,
собранной из правил анатомии и моделирования (`crownai/posterior_model.py`),
так же, как техник моделирует бугор за бугром.

**Морфология.**

- Каждый бугор — скруглённый конус на своём анатомическом месте.
- Фиссуры получаются сами там, где сходятся скаты соседних бугров. Поэтому
  рисунок фиссур у каждого зуба свой:
  - «Y» у нижней пятёрки;
  - пять бугров у нижней шестёрки;
  - крест у нижней семёрки;
  - косой валик у верхней шестёрки.
- От каждого бугра к центральной фиссуре идёт треугольный валик. Вдоль края
  зуба бугры переходят в пологие мезиальные и дистальные гребни.
- Краевые валики примерно на 1,2 мм ниже бугров, мезиальный выше дистального.
- Щёчный экватор в пришеечной трети, язычный в средней.
- Нижние жевательные зубы наклонены язычно, их язычные бугры выше щёчных.
- У верхних моляров самый крупный мезионёбный бугор, контур ромбовидный.
- Размеры взяты по таблице Уилера.

**Постановка по правилам.**

- Ширина — промежуток между соседями. Если промежутка нет, то ширина
  симметричного зуба, притянутая к учебной; если и его нет, то учебная.
  Щёчно-язычный размер — в учебной пропорции к мезиальному соседу.
- Коронка стоит вплотную к контакту с соседом. Бугры на уровне бугров
  соседей, затем опорные бугры доводятся до контакта с антагонистом.
- Каждый бугор поднимается или опускается отдельно, срезов нет.
- Рельеф края культи затухает к середине высоты коронки, поэтому жевательная
  поверхность не наклоняется за краем препарирования.
- Для имплантата и промежуточной части, где корня нет, зуб ставится на линию
  ряда соседей.

Режим с отражённым зубом остаётся: `--posterior-anatomy mirror`. Если в
`--library` есть выученная анатомия для этого класса зубов, используется она.

**Проверка против работ техника.** Среднее отклонение поверхности техника от
коронки (у 47 через дробь указаны обе стороны сравнения):

| Зуб | Анатомия по правилам | Прежний вариант |
| --- | --- | --- |
| 15 (культя) | **0,37 мм** | 0,49 мм (библиотека) |
| 16 (культя) | **0,41 мм** | 0,46 мм (библиотека) |
| 24 (культя) | 0,67 мм | **0,52 мм** (библиотека) |
| 25 (культя) | 0,60 мм | **0,56 мм** (библиотека) |
| 47 (имплантат) | 0,49 / 0,52 мм | **0,36 / 0,40 мм** (отражённый 37, выровненный по антагонисту) |

### Дообучение правил на архиве лаборатории

Учебные правила задают правильную, но обезличенную форму. Живость зубу дают
то, как лепят ваши техники, и вторичная анатомия: дополнительные бороздки и
форма валиков. Команда `tune-rules` учит и то и другое на готовых коронках из
архива:

```bash
crownai tune-rules Y:/CAD-Data_2025 --out rules_profile.json
crownai construction-case <папка кейса> --rules-profile rules_profile.json ...
```

Что делает команда с каждой коронкой техника (кейс с `.constructionInfo` и
коронкой, экспортированной обратно в папку):

1. Отделяет зуб от соседей по перешейку в контакте. Это нужно для
   сплинтованных ваксапов и мостов.
2. Вписывает модель по правилам в жевательную поверхность: высоты и положения
   бугров, скаты, треугольные валики, глубину фиссур, краевые валики.
3. Измеряет контур: экваторы, контакты, пропорции, наклон, «квадратность».
4. Сохраняет остаток, то есть поверхность техника минус подогнанные правила.
   Это вторичная анатомия.

Дальше по каждому типу зуба берётся медиана. Если коронок меньше пяти, тип
остаётся учебным. Часть коронок (15%) откладывается для проверки: они
моделируются с профилем и без него и сравниваются с техником. Итог
печатается и записывается в профиль.

Прогресс пишется в `rules_profile.fits.jsonl`, прерванный запуск
продолжается с места. В файлах только хэши кейсов, без имён и путей.

Проверка на двух верхних пятёрках из HTML (учимся на одной, моделируем
другую, отклонение от техника):
...
| `crownai/posterior_model.py` | премоляры и моляры по правилам анатомии: бугры, валики, фиссуры |
| `crownai/rules_tuning.py` | дообучение правил и вторичной анатомии на коронках лаборатории |
